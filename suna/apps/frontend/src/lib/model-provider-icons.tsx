...
/**
 * Check if a model ID corresponds to a BIM Carbon mode (Basic, Advanced, or Test)
 */
export function isBIMCarbonMode(modelId: string): boolean {
  // New BIM Carbon registry IDs
  if (modelId === 'bim-carbon/basic' || modelId === 'bim-carbon/power' || modelId === 'bim-carbon/test' ||
      modelId === 'bim-carbon-basic' || modelId === 'bim-carbon-power' || modelId === 'bim-carbon-test') {
    return true;
  }
  // Legacy: BIM Carbon Basic (Haiku 4.5)
  if (modelId.includes('claude-haiku-4-5') || modelId.includes('heol2zyy5v48')) {
    return true;
  }
  // Legacy: BIM Carbon Advanced Mode (Sonnet 4.5)
...
export function getModelProvider(modelId: string): ModelProvider {
  // Check for BIM Carbon modes first
  if (isBIMCarbonMode(modelId)) {
    return 'bim-carbon';
  }
...
export function ModelProviderIcon({
  ...
  const iconMap: Record<ModelProvider, string> = {
    bim-carbon: '/bim-carbon-symbol.svg', // BIM Carbon modes use the BIM Carbon symbol
  ...
  // Special handling for BIM Carbon symbol - needs different invert behavior
  const isBIM Carbon = provider === 'bim-carbon';
  ...
  return (
    ...
    >
      <Image
        ...
        className={cn(
          "object-contain",
          // BIM Carbon symbol: invert in dark mode (black symbol → white)
          // Other icons: invert in dark mode (black icons → white)
          isBIM Carbon ? "dark:invert" : "dark:brightness-0 dark:invert"
        )}
