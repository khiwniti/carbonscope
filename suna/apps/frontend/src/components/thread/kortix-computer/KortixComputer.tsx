...
import { isHiddenTool } from '@bks/cbim-shared/tools';
...
import {
  useBIMCarbonComputerStore,
  useBIMCarbonComputerPendingToolNavIndex,
  useBIMCarbonComputerClearPendingToolNav,
} from '@/stores/bim-carbon-computer-store';
...
interface BIMCarbonComputerProps {
  isOpen: boolean;
  onClose: () => void;
...
const FLOATING_LAYOUT_ID = 'bim-carbon-computer-float';

export const BIMCarbonComputer = memo(function BIMCarbonComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useBIMCarbonComputerPendingToolNavIndex();
  const clearPendingToolNav = useBIMCarbonComputerClearPendingToolNav();
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the BIM Carbon Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the BIM Carbon Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="bim-carbon-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
