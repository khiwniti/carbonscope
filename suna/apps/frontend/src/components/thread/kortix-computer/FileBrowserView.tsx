...
import { normalizeFilenameToNFC } from '@bks/cbim-shared';
import { cn } from '@/lib/utils';
import { useBIMCarbonComputerStore } from '@/stores/bim-carbon-computer-store';
import { usePresentationViewerStore } from '@/stores/presentation-viewer-store';
import { Badge } from '@/components/ui/badge';
import { VersionBanner } from './VersionBanner';
import { BIMCarbonComputerHeader } from './BIMCarbonComputerHeader';
...
interface FileBrowserViewProps {
  ...
  projectId?: string;
  /** 
   * 'default' - Side panel view in BIM Carbon Computer (shows all files)
      ...
      if (file.is_dir) {
        ...
        if (isDirectChildOfPresentations(file.path)) {
          ...
          if (isLibraryView && project?.sandbox?.sandbox_url) {
            ...
            // In side panel view, use bim-carbon computer store
            openFile(file.path);
          }
        } else {
        ...
        // Open file using bim-carbon computer store
      ...
      <div className="flex flex-col min-h-screen bg-background">
        ...
        <Dialog open={revertModalOpen} onOpenChange={setRevertModalOpen}>
          <DialogContent className="sm:max-w-md rounded-xl bg-background border border-border">
            ...
            {revertLoadingInfo ? (
              <div className="py-6 flex items-center justify-center"><CarbonScopeLoader size="medium" /></div>
            ) : revertCommitInfo ? (
              <div className="mt-2">
            ...
            <DialogFooter>
              ...
              <Button onClick={performRevert} disabled={revertInProgress}>
                {revertInProgress ? (<><CarbonScopeLoader size="small" className="mr-2" />Restoring...</>) : 'Restore'}
              </Button>
            </DialogFooter>
            <DialogClose />
          </DialogContent>
