...
import { useBIMCarbonComputerStore, useSetIsSidePanelOpen } from '@/stores/bim-carbon-computer-store';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const openFileInComputer = useBIMCarbonComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = useBIMCarbonComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = useBIMCarbonComputerStore((state) => state.setSandboxContext);
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show BIMCarbonComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show BIMCarbonComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | BIM Carbon`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive Worker conversation powered by BKS cBIM AI`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | BIM Carbon`);
      }
