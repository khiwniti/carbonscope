...
// Reusable agent header - shows BIM Carbon logo for BIM Carbon, avatar+name for others
const AgentHeader = memo(function AgentHeader({ agentInfo }: { agentInfo: AgentInfo }) {
  if (agentInfo.name === "BIM Carbon") {
    return (
      <img
        src="/bim-carbon-logomark-white.svg"
        alt="BIM Carbon"
...
const AssistantGroupRow = memo(function AssistantGroupRow({
  ...
  return (
    <div key={group.key} ref={isLastGroup ? latestMessageRef : null}>
      <div className="flex flex-col gap-2">
        {/* Reasoning section with integrated BIM Carbon icon */}
        {reasoningSection}
...
export const ThreadContent: React.FC<ThreadContentProps> = memo(
  function ThreadContent({
    ...
    project,
    isPreviewMode = false,
    agentName = "BIM Carbon",
    agentAvatar = <BIMCarbonLogo size={14} />,
    emptyStateComponent,
    threadMetadata,
    scrollContainerRef,
    threadId,
    ...
    const agentInfo = useMemo<AgentInfo>(() => {
      ...
      if (recentAssistantWithAgent?.agents?.name) {
        ...
        const name =
          typeof rawName === "string" ? rawName : String(rawName || "BIM Carbon");
        return {
          name,
          avatar: (
            <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
              <BIMCarbonLogo size={14} />
            </div>
          ),
        };
      }
      const fallbackName = typeof agentName === "string" ? agentName : "BIM Carbon";
      return {
        name: fallbackName || "BIM Carbon",
        avatar: agentAvatar,
      };
