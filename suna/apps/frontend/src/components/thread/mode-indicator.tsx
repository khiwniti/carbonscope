...
const ModeLogo = memo(function ModeLogo({ 
  ...
  return (
    <span className="flex-shrink-0 relative" style={{ height: `${height}px`, width: 'auto' }}>
      ...
      <img
        src={lightSrc}
        alt={mode === 'advanced' ? 'BIM Carbon Advanced' : 'BIM Carbon Basic'}
...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'bim-carbon/basic' || m.label === 'BIM Carbon Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'bim-carbon/power' || m.label === 'BIM Carbon Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'bim-carbon/basic' && m.id !== 'bim-carbon/power' && 
             m.label !== 'BIM Carbon Basic' && m.label !== 'BIM Carbon Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access BIM Carbon Advanced mode',
        });
      }
    }
