...
// BIM Carbon agent modes switcher - isolated from typing state
...
interface SubmitButtonProps {
  ...
  buttonLoaderVariant: 'emerald' | 'white';
        ...
        <TooltipTrigger asChild>
          ...
          >
            {((loading || isUploading) && !isAgentRunning) ? (
              <CarbonScopeLoader size="small" customSize={20} variant={buttonLoaderVariant} />
            ) : showAddToQueue ? (
...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const [isModeDismissing, setIsModeDismissing] = useState(false);    // BIM Carbon Agent Modes feature flag
    ...
    // Dark theme → light button → needs emerald loader
    // Light theme → dark button → needs white loader
    const { resolvedTheme } = useTheme();
    const buttonLoaderVariant = (resolvedTheme === 'dark' ? 'emerald' : 'white') as 'emerald' | 'white';
    ...
    // Check if selected agent is BIM Carbon based on agent data
    // While loading, default to BIM Carbon (assume BIM Carbon is the default agent)
    const selectedAgent = agents.find(agent => agent.agent_id === selectedAgentId);
    const sunaAgent = agents.find(agent => agent.metadata?.is_suna_default === true);
    const isSunaAgent = isLoadingAgents 
        ? true // Show BIM Carbon modes while loading
