...
import { BIMCarbonComputer, ToolCallInput } from '@/components/thread/bim-carbon-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // BIM Carbon Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useBIMCarbonComputerStore();
  ...
  // Handle file click - now opens in BIM Carbon Computer instead of modal
  ...
  const selectedFilePath = useBIMCarbonComputerStore((state) => state.selectedFilePath);
