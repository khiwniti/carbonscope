...
// Helper to render model labels with special styling for BIM Carbon modes
const ModelLabel = ({ label, className }: { label: string; className?: string }) => {
    if (label === 'BIM Carbon Advanced Mode') {
        return (
            <span className={cn("flex items-center gap-2", className)}>
                <span className="font-medium">BIM Carbon</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/10 dark:bg-primary/15 rounded-full">
                    <BIMCarbonLogo size={12} variant="symbol" />
...
export function AgentModelSelector({
  ...
  const handleSelect = (modelId: string) => {
    ...
    if (hasAccess) {
      ...
      const isPowerModel = modelId === 'bim-carbon/power';
      openPricingModal({
        isAlert: true,
        alertTitle: isPowerModel ? 'Upgrade to access BIM Carbon Advanced mode' : 'Upgrade to access this model',
      });
    }
  };
  ...
  const renderModelOption = (model: any, index: number) => {
    ...
    const isPowerModel = model.id === 'bim-carbon/power';
        ...
        <TooltipTrigger asChild>
          ...
          {!accessible && !isLocalMode() ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{isPowerModel ? 'Upgrade to access BIM Carbon Advanced mode' : 'Upgrade to access this model'}</p>
            </TooltipContent>
          ) : isLowQuality ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
