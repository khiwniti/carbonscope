  ...
  useEffect(() => {
    if (template?.icon_name && template?.icon_background) {
      ...
      if (imageRef.current && imageLoaded) {
        (async () => {
          try {
            // @ts-ignore - Dynamic import for browser-specific build
            const ColorThief = (await import('colorthief/dist/color-thief.mjs')).default;
