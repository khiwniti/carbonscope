...
export async function GET(request: NextRequest) {
  ...
  // In production, always use NEXT_PUBLIC_URL to prevent localhost redirects
  // Only fall back to request origin in development
  const isProd = process.env.NEXT_PUBLIC_ENV_MODE === 'production'
  const configuredUrl = process.env.NEXT_PUBLIC_URL
  const requestOrigin = request.nextUrl.origin
  const baseUrl = isProd && configuredUrl ? configuredUrl : (requestOrigin || configuredUrl || 'http://localhost:3000')
