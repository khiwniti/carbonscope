...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_bim-carbon_team !== undefined) searchParams.set('is_bim-carbon_team', params.is_bim-carbon_team.toString());
...
export function useBIMCarbonTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'bim-carbon-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view BIM Carbon templates');
      }

      const response = await fetch(`${API_URL}/templates/bim-carbon-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
