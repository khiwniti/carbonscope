...
import { ToolCallInput } from '@/components/thread/bim-carbon-computer';
...
import { useBIMCarbonComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/bim-carbon-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useBIMCarbonComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure BIMCarbonComputer switches to tools view
