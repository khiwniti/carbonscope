...
const getDefaultModel = (accessibleModels: ModelOption[]): string => {
  ...
  // bim-carbon/basic should be first for free users since power is not accessible
  const basicModel = accessibleModels.find(m => m.id === 'bim-carbon/basic');
  if (basicModel) return basicModel.id;

  const powerModel = accessibleModels.find(m => m.id === 'bim-carbon/power');
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      const powerModel = availableModels.find(m => m.id === 'bim-carbon/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to bim-carbon/power');
        setSelectedModel('bim-carbon/power');
      }
    }
