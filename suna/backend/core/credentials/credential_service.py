...
class EncryptionService:
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        key_env = os.getenv("MCP_CREDENTIAL_ENCRYPTION_KEY") or os.getenv(
            "ENCRYPTION_KEY"
        )
        ...
        if not key_env:
            ...
            raise ValueError(
                "MCP_CREDENTIAL_ENCRYPTION_KEY or ENCRYPTION_KEY must be set"
            )

        # Ensure the key is a valid Fernet key (44 url-safe base64 characters)
        try:
            ...
            Fernet(key_str.encode())  # will raise if invalid
            return key_str.encode("utf-8")
        except Exception as e:
            logger.warning(
                f"Provided encryption key invalid ({e}), generating a temporary key for testing."
            )
            # Generate a temporary Fernet key for fallback in tests
            from cryptography.fernet import Fernet
    ...
    def decrypt_config(
        self, encrypted_config: bytes, expected_hash: str
    ) -> Dict[str, Any]:
        try:
...
class CredentialService:
    ...
    async def store_credential(
        ...
        if existing.data:
            await (
                client.table("user_mcp_credentials")
                .update(
                    {
                        "is_active": False,
                ...
                .eq("credential_id", existing.data[0]["credential_id"])
                .execute()
            )
    ...
    async def get_missing_credentials(
        self, account_id: str, requirements: List[MCPRequirement]
    ) -> List[MCPRequirement]:
        ...
        for req in requirements:
            if req.custom_type:
                ...
                for cred in user_credentials:
                    if (
                        cred.mcp_qualified_name.startswith(custom_pattern)
                        and req.display_name.lower().replace(" ", "_")
                        in cred.mcp_qualified_name
                    ):
    ...
    def _map_to_credential(self, data: Dict[str, Any]) -> MCPCredential:
        try:
            ...
            config = self._encryption.decrypt_config(
                encrypted_config, data["config_hash"]
            )
        except Exception as e:
        ...
        return MCPCredential(
            ...
            last_used_at=datetime.fromisoformat(
                data["last_used_at"].replace("Z", "+00:00")
            )
