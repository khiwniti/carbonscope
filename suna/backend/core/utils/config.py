...
class Configuration:
    ...
    STRIPE_TIER_2_17_YEARLY_COMMITMENT_ID_PROD: Optional[str] = (
        "price_1RqtqiG6l1KZGqIrhjVPtE1s"  # $17/month
    )
    STRIPE_TIER_6_42_YEARLY_COMMITMENT_ID_PROD: Optional[str] = (
        "price_1Rqtr8G6l1KZGqIrQ0ql0qHi"  # $42.50/month
    )
    STRIPE_TIER_25_170_YEARLY_COMMITMENT_ID_PROD: Optional[str] = (
        "price_1RqtrUG6l1KZGqIrEb8hLsk3"  # $170/month
    )
    ...
    STRIPE_TIER_2_17_YEARLY_COMMITMENT_ID_STAGING: Optional[str] = (
        "price_1RqYGaG6l1KZGqIrIzcdPzeQ"  # $17/month
    )
    STRIPE_TIER_6_42_YEARLY_COMMITMENT_ID_STAGING: Optional[str] = (
        "price_1RqYH1G6l1KZGqIrWDKh8xIU"  # $42.50/month
    )
    STRIPE_TIER_25_170_YEARLY_COMMITMENT_ID_STAGING: Optional[str] = (
        "price_1RqYHbG6l1KZGqIrAUVf8KpG"  # $170/month
    )
    ...
    GA_CREDENTIALS_JSON: Optional[str] = (
        None  # Service account JSON credentials (as string or file path)
    )
    ...
    SUPABASE_WEBHOOK_SECRET: Optional[str] = (
        None  # Secret for Supabase database webhook authentication
    )
    ...
    def MAX_PARALLEL_AGENT_RUNS(self) -> int:
        ...
        if self._MAX_PARALLEL_AGENT_RUNS_ENV is not None:
            try:
                ...
                logger.warning(
                    f"Invalid MAX_PARALLEL_AGENT_RUNS value: {self._MAX_PARALLEL_AGENT_RUNS_ENV}, using default"
                )
    ...
    def __init__(self):
        ...
        self._load_from_env()
        # Ensure Braintrust API key exists for eval tests
        ...
        if not self.KORTIX_ADMIN_API_KEY:
            ...
            logger.info(
                "Auto-generated KORTIX_ADMIN_API_KEY for administrative functions"
            )
    ...
    def _load_from_env(self):
        ...
        if debug_save_llm_io_env is not None:
            self._DEBUG_SAVE_LLM_IO = debug_save_llm_io_env.lower() in (
                "true",
                "t",
                "yes",
                "y",
