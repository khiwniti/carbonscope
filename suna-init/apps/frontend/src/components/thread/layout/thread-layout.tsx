...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // CarbonScope Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useCarbonScopeComputerStore();
  ...
  // Handle file click - now opens in CarbonScope Computer instead of modal
  ...
  const selectedFilePath = useCarbonScopeComputerStore((state) => state.selectedFilePath);
