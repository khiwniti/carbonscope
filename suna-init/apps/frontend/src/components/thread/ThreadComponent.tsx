...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const openFileInComputer = useCarbonScopeComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = useCarbonScopeComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = useCarbonScopeComputerStore((state) => state.setSandboxContext);
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show CarbonScopeComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show CarbonScopeComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | CarbonScope`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive Worker conversation powered by CarbonScope`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | CarbonScope`);
      }
