...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_CarbonScope_team !== undefined) searchParams.set('is_CarbonScope_team', params.is_CarbonScope_team.toString());
...
export function useCarbonScopeTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'CarbonScope-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view CarbonScope templates');
      }

      const response = await fetch(`${API_URL}/templates/CarbonScope-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
