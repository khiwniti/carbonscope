...
# Only initialize Daytona client if credentials are present
if daytona_config.api_key and daytona_config.api_url:
    daytona = AsyncDaytona(daytona_config)
    logger.info("Daytona client initialized successfully")
else:
    daytona = None
    logger.warning("Daytona client not initialized - missing credentials. Sandbox features will be unavailable.")

async def get_or_start_sandbox(sandbox_id: str) -> AsyncSandbox:
    """Retrieve a sandbox by ID, check its state, and start it if needed."""

    if daytona is None:
        raise RuntimeError("Daytona client not initialized - sandbox features unavailable")
