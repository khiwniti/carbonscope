...
async def verify_admin_api_key(x_admin_api_key: Optional[str] = Header(None)):
    if not config.CarbonScope_ADMIN_API_KEY:
        raise HTTPException(
            status_code=500,
    ...
    if not _constant_time_compare(x_admin_api_key, config.CarbonScope_ADMIN_API_KEY):
        raise HTTPException(
            status_code=403,
...
async def verify_and_get_user_id_from_jwt(request: Request) -> str:
    ...
    if os.getenv("DEV_AUTH_BYPASS") == "true":
        # Check for special dev header or test email in authorization
        dev_test_header = request.headers.get('x-dev-test-user')
        auth_header = request.headers.get('Authorization')

        # Allow bypass via special header
        if dev_test_header == "true":
            test_user_id = os.getenv("DEV_TEST_USER_ID", "00000000-0000-0000-0000-000000000001")
            logger.warning(f"🚨 DEV_AUTH_BYPASS: Using test user {test_user_id} (x-dev-test-user header)")
            structlog.contextvars.bind_contextvars(
                user_id=test_user_id,
                auth_method="dev_bypass_header"
            )
            return test_user_id

        # Allow bypass via test email in Bearer token (any token with format "dev:test@dev.local")
        if auth_header and auth_header.startswith('Bearer dev:'):
            test_email = auth_header.replace('Bearer dev:', '')
            if test_email == os.getenv("DEV_TEST_EMAIL", "test@dev.local"):
                test_user_id = os.getenv("DEV_TEST_USER_ID", "00000000-0000-0000-0000-000000000001")
                logger.warning(f"🚨 DEV_AUTH_BYPASS: Using test user {test_user_id} (test email: {test_email})")
                structlog.contextvars.bind_contextvars(
                    user_id=test_user_id,
                    auth_method="dev_bypass_email"
                )
                return test_user_id
