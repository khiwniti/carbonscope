...
api_router.include_router(carbon_api.router)
...
async def health_check():
    ...
    try:
        ...
        # Count active agents (will increase in Wave 2 as agents are added)
        # For now, just verify the infrastructure works
        active_agents = 0  # Will be populated with actual agent count in Wave 2

    except Exception as e:
        agent_system_healthy = False
        logger.error(f"Agent health check failed: {e}")

    overall_status = "ok" if agent_system_healthy else "degraded"
